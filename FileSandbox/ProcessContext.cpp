						...
						if (ReadProcessMemory(Process, (LPCVOID)Event->u.Exception.ExceptionRecord.ExceptionInformation[0], &MyExceptionSEH, sizeof(MyExceptionSEH), &BytesRead))
						{
							switch (Event->u.Exception.ExceptionRecord.ExceptionCode)
							{
								...
								case SANDBOX_API_PROCESS_CREATEUSERPROCESS:
								{
									...
									{
										Who = RemoteStructureRoutine::RemoteReadUserProcessParameters(Process, (LPVOID*) RemoteStructureRoutine::RemotePointerGet(Process, (VOID*) ArgumentList[FILESANDBOX_NTCREATEUSERPROCESS_PROCESSPARAMETERS], sizeof(VOID*)));
										if (Who != nullptr)
										{
